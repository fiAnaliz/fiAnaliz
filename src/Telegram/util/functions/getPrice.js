...
exports.stockMarket = async function get_price_stock(stock, whereIs, client, message){
    if(whereIs === 'BIST'){
        ...
        if (err) { return console.log(err); }
            ...
            client.sendMessage(message.chat.id, reply, {parse_mode: 'Markdown'})
        })
    }else if(whereIs === 'US'){
...
Crypto Currency
*/
exports.coin = async function crypto_price_get(cryptocurrency, client, message){
...
Currencies Module
*/
exports.currencies = async function currency_rate(currency, client, message){
    var today = new Date()
    var day = today.getDate().toString()
    var month = (today.getMonth() + 1).toString()
    var year = today.getFullYear().toString() 
    ...
    if (err) { return console.log(err); }
        ...
        var lastData = data.dataSet[data.dataSet.length-1]
        var change = (lastData.close - data.dataSet[data.dataSet.length-2].close) / data.dataSet[data.dataSet.length-2].close *100
        reply = `*${Config.DOVIZ_description[Config.DOVIZ_symbols.indexOf(currency)].toUpperCase()}* | ${currency}\n\n*Fiyat:* _${lastData.close.toFixed(2)} ₺_ | *Değişim:* _% ${change.toFixed(2)}_ \n*Düşük:* _${lastData.low.toFixed(2)} ₺_ | *Yüksek:* _${lastData.high.toFixed(2)} ₺_`
        client.sendMessage(message.chat.id, reply, {parse_mode: 'Markdown'})
    })
}
...
exports.index = async function index_worth(indextype, client, message){
    var today = new Date()
    var day = today.getDate().toString()
    var month = (today.getMonth() + 1).toString()
    var year = today.getFullYear().toString() 
    ...
    if (err) { return console.log(err); }
        ...
        var lastData = data.dataSet[data.dataSet.length-1]
        var change = (lastData.close - data.dataSet[data.dataSet.length-2].close) / data.dataSet[data.dataSet.length-2].close *100
        reply = `*${Config.ENDEKS_description[Config.ENDEKS_symbols.indexOf(indextype)].toUpperCase()}* | ${indextype}\n\n*Fiyat:* _${lastData.close.toFixed(2)}_ | *Değişim:* _% ${change.toFixed(2)}_ \n*Düşük:* _${lastData.low.toFixed(2)}_ | *Yüksek:* _${lastData.high.toFixed(2)}_`
        client.sendMessage(message.chat.id, reply, {parse_mode: 'Markdown'})
    })
}
...
Gold Module
*/
exports.gold = async function gold(goldtype, client, message){
    var today = new Date()
    var day = today.getDate().toString()
    var month = (today.getMonth() + 1).toString()
    var year = today.getFullYear().toString() 
    ...
    if (err) { return console.log(err); }
        ...
        var lastData = data.dataSet[data.dataSet.length-1]
        var change = (lastData.close - data.dataSet[data.dataSet.length-2].close) / data.dataSet[data.dataSet.length-2].close *100
        reply = `*${Config.GOLD_description[Config.GOLD_symbols.indexOf(goldtype)].toUpperCase()}* | ${goldtype}\n\n*Fiyat:* _${lastData.close.toFixed(2)}_ | *Değişim:* _% ${change.toFixed(2)}_ \n*Düşük:* _${lastData.low.toFixed(2)}_ | *Yüksek:* _${lastData.high.toFixed(2)}_`
        client.sendMessage(message.chat.id, reply, {parse_mode: 'Markdown'})
    })
}